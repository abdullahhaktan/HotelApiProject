...
namespace HotelProject.WebUI.Controllers
{
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> AddStaff(CreateStaffDto createStaffDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5123/api/Staff", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5123/api/Staff/{id}"); //Staff?id={id} yazarak da olur du
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
